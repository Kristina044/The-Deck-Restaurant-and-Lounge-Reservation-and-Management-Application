...
class ProductController extends GetxController {
  ...
  Future<List<Product>> getProductList() async {
    final response =
        await http.get(Uri.parse('http://172.25.40.14:8080/product'));
  ...
  Future<void> getProductById(int id) async {
    final response =
        await http.get(Uri.parse('http://172.25.40.14:8080/product/${id}'));
  ...
  Future<void> getProductByCategoryId(int categoryId) async {
    final response = await http.get(
        Uri.parse('http://172.25.40.14:8080/product/category/$categoryId'));
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body)['message'];
