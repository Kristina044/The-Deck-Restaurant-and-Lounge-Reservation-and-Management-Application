...
class RegisterController extends GetxController {
  ...
  Future<void> registerUser(RegisterModel registerModel) async {
    final url = Uri.parse('http://172.25.40.14:8080/customer/register');
  ...
  Future<void> loginUser(String email, String password) async {
    final url = Uri.parse('http://172.25.40.14:8080/customer/login');
  ...
  Future<void> getUserProfile() async {
    final url = Uri.parse('http://172.25.40.14:8080/customer/profile');
  ...
  Future<void> logout() async {
    final url = Uri.parse('http://172.25.40.14:8080/customer/logout');
    final token = box.read('token');
    final response = await http.post(
      url,
      headers: {'Cookie': 'jwt=$token'},
  ...
  Future<void> addToCart(int productId, int quantity) async {
    final url = Uri.parse('http://172.25.40.14:8080/cart/add');
    final token = box.read('token');
    final response = await http.post(
      url,
      headers: {'Cookie': 'jwt=$token'},
  ...
  final url = Uri.parse('http://172.25.40.14:8080/cart/myCart');
  final token = box.read('token');
  final response = await http.get(
    url,
    headers: {'Cookie': 'jwt=$token'},
  ...
  if (response.statusCode == 200) {
    ...
    if (data is List<dynamic>) {
      List<CartItem> items = data.map((json) => CartItem.fromJson(json)).toList();
  ...
  Future<void> deleteCartItem(int cartItemId) async {
    final url = Uri.parse('http://172.25.40.14:8080/cart/delete/$cartItemId');
    final token = box.read('token');
  ...
  Future<void> updateCartItemQuantity(int cartItemId, int quantity) async {
    final url = Uri.parse('http://172.25.40.14:8080/cart/edit/$cartItemId');
    final token = box.read('token');
  ...
  Future<void> updateUserProfile(Customer updatedCustomer) async {
    final url = Uri.parse('http://172.25.40.14:8080/customer/update-profile');
    final token = box.read('token');
    final response = await http.put(
      url,
      headers: {
